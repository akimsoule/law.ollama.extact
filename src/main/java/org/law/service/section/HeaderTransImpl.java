...
import static org.law.service.section.Constant.*;

public class HeaderTransImpl implements BoolString, ExtractString, TransString, TemplateReader {
    ...
    public String cleanUpHeader(String header) throws Exception {
        ...
        for (String line : lines) {
            ...
            if (cleanLine.length() <= 2) continue; // Ignore le bruit (D, z, X, ')
            ...
            else if (ASSEMBLEE_NATIONALE.stream().anyMatch(token ->
                    containTarget(cleanLine, token, 3))) {
                state = "DELIBERATION";
            }
        ...
        // Utilise tes méthodes de réparation sur le numéro de loi (qui contient la date)
